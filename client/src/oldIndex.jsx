...
class App extends React.Component {
  constructor(props) {
    ...
    this.assignAndSave = this.assignAndSave.bind(this);
  ...
  check(cacheName, id) {
    return this.cache[cacheName].get(id);
  }

  cache(cacheName, id, value) {
    this.cache[cacheName].set(id, value);
  }
  ...
  comeHome() {
    const { household, workload, addMember } = this;
    this.setState({
      nav: <Dashboard household={household} workload={workload} onClick={(name) => this.memberDashboard(name)} onUpdate={addMember} />
    });
  }
  ...
  addMember(member) {
    console.log(member);
    console.log('Attempting to save new household member:', member.name);
...
getHouseholdData(() => {
  ReactDOM.render(<App household={} />, document.getElementById('App'));
});
